...
def Main_Default(value_max: int, num_loops: int, rlock):
    ...
    txt_output.write("Average time it took to calculate {0} compiled default LRU passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
...
def Main_Half(value_max: int, num_loops: int, rlock):
    ...
    my_file2 = "files_runs/compiled_half_LRU_division.txt"
    txt_output = open(my_file, "a")
    txt_output2 = open(my_file2, "a")
    time_list = []
    divisions_list = []
    ...
    average_time = ft.reduce(lambda a, b: a + b, time_list) / len(time_list)
    txt_output.write("Average time it took to calculate {0} compiled half LRU passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
...
def Main_Sqrt(value_max: int, num_loops: int, rlock):
    ...
    average_time = ft.reduce(lambda a, b: a + b, time_list) / len(time_list)
    txt_output.write("Average time it took to calculate {0} compiled sqrt LRU passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
