...
def Main_Default(value_max, num_loops, rlock):
    ...
    txt_output.write("Average time it took to calculate {0} compiled default passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
...
def Main_Half(value_max, num_loops, rlock):
    ...
    average_time = ft.reduce(lambda a, b: a + b, time_list) / len(time_list)
    txt_output.write("Average time it took to calculate {0} compiled half-bound passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
...
def Main_Sqrt(value_max, num_loops, rlock):
    ...
    average_time = ft.reduce(lambda a, b: a + b, time_list) / len(time_list)
    txt_output.write("Average time it took to calculate {0} compiled sqrt-bound passes was {1} seconds.".format(num_loops, average_time))
    txt_output.close()
