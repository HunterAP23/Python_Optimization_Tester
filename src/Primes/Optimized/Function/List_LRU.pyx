...
def is_prime_default(n: int, table: list):
    cdef list ret = [n % table[i] for i in range(len(table))]
...
def is_prime_half(n: int, table: list):
    ...
    cdef list ret = [n % table[i] for i in range(len(table)) if table[i] <= boundary]
    return (all(ret), sum([bool(i) for i in ret]))


@cython.cfunc
@cython.locals(n=cython.int, i=cython.int, boundary=cython.int)
@ft.lru_cache(maxsize=None)
