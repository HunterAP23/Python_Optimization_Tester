...
#         tmp_time_total = time.time() - tmp_time_start
#
#         time_output.write("CPython Interpreted (Function List LRU) Pass {0} took {1} seconds.\n\n".format(i + 1, tmp_time_total))
#         time_list.append(tmp_time_total)
#
...
#     time_now = dt.datetime.now()
#     msg = ("-" * 80) + "\n"
#     msg += "CPython Interpreted (Function List LRU) finished at {0}/{1}/{2} {3}:{4}:{5}:{6}".format(time_now.year, time_now.month, time_now.day, time_now.hour, time_now.minute, time_now.second, time_now.microsecond)
#     print_lock(msg, rlock)
#
#     average_time = ft.reduce(lambda a, b: a + b, time_list) / len(time_list)
