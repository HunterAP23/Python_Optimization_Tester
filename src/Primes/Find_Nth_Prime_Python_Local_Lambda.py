...
def main_def(my_max, num_loops, rlock):
    ...
    time_output = open("files_runs/normal_local_lambda/default_time.txt", "w")
    ...
    for i in range(num_loops):
        ...
        for n in range(3, my_max, 2):
            if all(n % primes_list[i] for i in range(len(primes_list))):
    ...
    with open("files_runs/normal_local_lambda/default_divisions.txt", "w") as div_output:
        for div in div_list:
            div_output.write(div)

    with open("files_runs/normal_local_lambda/default_primes.txt", "w") as primes_output:
...
def main_half(my_max, num_loops, rlock):
    ...
    for i in range(num_loops):
        ...
        for n in range(3, my_max, 2):
            ...
            if all(n % primes_list[i] for i in range(len(primes_list)) if primes_list[i] <= boundary):
                div_list.append("Primality Test for {0} took {1} divisions.\n\n".format(n, 1))
                primes_list.append(n)
