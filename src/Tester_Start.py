        ...
        if "global" not in config.keys():
            ...
            if "threads" not in config["global"]:
                ...
                try:
                    ...
                    print(
                        'ERROR: Value ({0}) for "threads" key is not an integer. Defaulting to all CPU threads ({1}).'.format(
                            config["global"]["threads"], mp.cpu_count()
                        )
                    )
                    threads = mp.cpu_count()
...
def parse_args():
    parser = argp.ArgumentParser(
        description="Run benchmarking test suite for combinations of CPython, Cython, Anaconda, and PyPy.",
    ...
    optional_args.add_argument(
        ...
        nargs="+",
        required=False,
        help="Override the list of test suites specified in the settings.json file with space/comma separated ones.\n",
    )
    optional_args.add_argument(
        "-t",
        "--threads",
        ...
        type=int,
        required=False,
        help="Number of CPU threads to utilize in the benchmarks (defaults to all threads).\n",
    )
    optional_args.add_argument(
        "-f",
        "--file",
        ...
        required=False,
        help="Specify different JSON file for getting settings.\n",
    )
    optional_args.add_argument(
        "-o",
        "--os",
    ...
    misc_args.add_argument(
        "-d", "--debug", dest="Debug", action="store_true", required=False, help="Print verbose statements.\n"
    )
    misc_args.add_argument("-v", "--version", action="version", version="%(prog)s 2020-07-10")
    misc_args.add_argument(
        "-h", "--help", action="help", default=argp.SUPPRESS, help="Show this help message and exit.\n"
    )
